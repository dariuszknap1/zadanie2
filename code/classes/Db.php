...
class Db
{
    ...
    {
        try {
            //$this->db = new mysqli("127.0.0.1:3306", "user", "password", "mydb" );
            $this->db = new PDO('mysql:host=127.0.0.1:3306;port=3306;dbname=mydb', 'root', "password");
    ...
    public function deleteMessage($id)
    {
        ...
        if (isset($_SESSION['delete message']) && isset($_SESSION['logged'])) {
            try {
                ...
                // $stmt->execute(['id' => $id]);
                // $msg_data = $stmt->fetch();
                // $prev_data = $id . "|" . $msg_data['name'] . "|" . $msg_data['type'] . "|" . $msg_data['message'] . "|" . $msg_data['deleted'];
                $prev_data = self::getMessageRowData($id);
                ...
                //get what changed in a database
                // $sql = "SELECT * FROM message WHERE id=:id";
                // $stmt = $this->db->prepare($sql);
                // $stmt->execute(['id' => $id]);
                // $msg_data = $stmt->fetch();
                // $new_data = $id . "|" . $msg_data['name'] . "|" . $msg_data['type'] . "|" . $msg_data['message'] . "|" . $msg_data['deleted'];
